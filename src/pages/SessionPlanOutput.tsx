...
const SessionPlanOutput = () => {
  ...
  const getInitialData = React.useCallback(() => {
    ...
    const initialData = {
      ...
      metadata: { 
        ...(data.metadata || {}),
        grade: formatGrade(data.metadata?.grade) // Format grade when initializing
      }
    };
  ...
  const handleSaveAll = async (source = 'top') => {
    ...
    try {
      const payload = {
        ...
        orgcode: 'OR01',
        usercode: 'UO01',
        classname: editableData.metadata?.grade || '',
      ...
      const response = await axios.post('https://ai.excelsoftcorp.com/aiapps/AIToolKit/LessonPlanGen/api/save_lesson_plan', JSON.stringify(payload), {
  ...
  const fetchImages = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get('https://ai.excelsoftcorp.com/aiapps/AIToolKit/LessonPlanGen/api/search-images', {
        params: { 
  ...
  const fetchVideos = async (topicobj, subjectobj, gradeobj) => {
      ...
      const grade = gradeobj ? parseInt(gradeobj) : null;
          ...
          if (topic) {
            const response = await axios.get('https://ai.excelsoftcorp.com/aiapps/AIToolKit/LessonPlanGen/api/search-videos', {
              params: { 
                topic: topic,
                subject: subject,
                grade: grade,

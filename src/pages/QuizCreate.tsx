          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {/* ELO Multi-Select */}
                {selectedChapter && elos.length > 0 && (
                  <div>
                    <Label>Expected Learning Outcomes</Label>
                    <div className="border border-input rounded-lg p-4 bg-background mt-2">
                      {eloLoading ? (
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center space-x-2 border-b border-border pb-2">
                            <Checkbox
                              id="select-all-elos"
                              checked={elos.length > 0 && elos.every(elo => elo.selected)}
                              onCheckedChange={(checked) => {
                                setElos(prev => prev.map(elo => ({ ...elo, selected: !!checked })));
                              }}
                              className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                            />
                            <Label htmlFor="select-all-elos" className="text-sm font-semibold cursor-pointer">
                              Select All
                            </Label>
                            {getSelectedELOsCount() > 0 && (
                              <Badge variant="secondary" className="ml-auto">
                                {getSelectedELOsCount()} selected
                              </Badge>
                            )}
                          </div>
                          <div className="space-y-2 max-h-48 overflow-y-auto">
                            {elos.map(elo => (
                              <div key={elo.id} className="flex items-start space-x-2 hover:bg-muted/50 p-2 rounded-md transition-colors">
                                <Checkbox
                                  id={`elo-${elo.id}`}
                                ...
                                <div className="flex-1">
                                  <Label htmlFor={`elo-${elo.id}`} className="text-sm font-medium cursor-pointer">
                                    {elo.title}
                                  </Label>
                                  <p className="text-xs text-muted-foreground mt-1">{elo.description}</p>
                                </div>
                              </div>
                            ))}
                          </div>
